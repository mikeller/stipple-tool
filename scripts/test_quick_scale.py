"""Quick test: 200 spheres from face 2, batch 50."""
...
for batch_start in range(0, TOTAL, BATCH):
    ...
    if cut.IsDone() and not cut.Shape().IsNull():
        ...
        if rv < 1e-6 or drop > max_drop:
            print(f"  FAIL batch {batch_start}: vol={rv:.1f} drop={drop:.1f} max_ok={max_drop:.1f}")
            break
        cur = result; cur_vol = rv
        elapsed = time.time() - t0
        print(f"  Batch {batch_start}-{batch_start+len(batch)}: vol={rv:.1f} drop={drop:.1f} t={elapsed:.1f}s")
        sys.stdout.flush()
    else:
        print(f"  FAIL batch {batch_start}: cut failed"); break
else:
    removed = orig_vol - cur_vol
    print(f"OK: {TOTAL} spheres, removed={removed:.1f}mm³, time={time.time()-t0:.1f}s")
