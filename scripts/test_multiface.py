"""Test compound batch cuts across MULTIPLE faces: round-robin across faces."""
...
# Collect ALL faces with triangulation
faces_data = []
...
print(f"Found {len(faces_data)} faces with triangulation")
...
# Generate spheres grouped by face
SPHERES_PER_FACE = int(sys.argv[1]) if len(sys.argv) > 1 else 50
BATCH = int(sys.argv[2]) if len(sys.argv) > 2 else 50
USE_FACES = min(int(sys.argv[3]) if len(sys.argv) > 3 else 13, len(faces_data))

face_spheres = {}
for fi in range(USE_FACES):
    fd = faces_data[fi]
    face_spheres[fi] = [sample_from_face(fd) for _ in range(SPHERES_PER_FACE)]

total = SPHERES_PER_FACE * USE_FACES
print(f"\n{total} total spheres ({SPHERES_PER_FACE}/face × {USE_FACES} faces), batch {BATCH}")
print(f"Strategy: process each face's spheres in batches of {BATCH}")
    ...
    for batch_start in range(0, len(spheres), BATCH):
        ...
        if cut.IsDone() and not cut.Shape().IsNull():
            ...
            if rv < 1e-6 or drop > max_drop:
                print(f"  FAIL face {fi} batch {batch_start}: vol={rv:.1f} drop={drop:.1f} max_ok={max_drop:.1f}")
                failed = True; break
            cur = result; cur_vol = rv
            face_applied += len(batch)
        else:
            print(f"  FAIL face {fi} batch {batch_start}: cut failed")
            failed = True; break
    
    total_applied += face_applied
    elapsed = time.time() - t0
    face_time = time.time() - face_start
    print(f"  Face {fi}: {face_applied}/{len(spheres)} applied, vol={cur_vol:.1f}, face_time={face_time:.1f}s, total={elapsed:.1f}s")
    sys.stdout.flush()
    if failed: break

elapsed = time.time() - t0
removed = orig_vol - cur_vol
print(f"\nTotal: {total_applied}/{total} applied, removed={removed:.1f}mm³, time={elapsed:.1f}s")
if not failed:
    print("SUCCESS")
